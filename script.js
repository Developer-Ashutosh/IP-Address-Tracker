...
const fetchData = async (ipAddress) => {
  try {
    ...
    updateData(ip, city, region, country_code, timezone, org);
    initializeMap(latitude, longitude, city);
  } catch (error) {
    displayError(
      "Failed to fetch data. Please check the entered IP address and try again."
    );
    ipAddressBox.textContent =
      locationBox.textContent =
      timezoneBox.textContent =
      ispBox.textContent =
        "N/A";
    searchClientIP();
  }
...
const updateData = (
  ...
  locationBox.textContent = regionName
    ? regionName +
...
const initializeMap = (latitude, longitude, city) => {
  ...
  L.marker([latitude, longitude])
    .bindPopup(`<p style="margin:0;">${city}.</p>`)
    .addTo(map);
